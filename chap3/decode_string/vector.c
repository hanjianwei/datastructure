...
enum Status vector_init(struct Vector *vec, int cap) {
  ...
  vec->buffer = (VectorDataType *)malloc(cap * sizeof(VectorDataType));
  if (vec->buffer == NULL) {
...
enum Status vector_reserve(struct Vector *vec, int new_cap) {
  ...
  VectorDataType *p =
      (VectorDataType *)realloc(vec->buffer, new_cap * sizeof(VectorDataType));
  if (p == NULL) {
    return STATUS_ERR_ALLOC;
  }
...
void vector_reverse(struct Vector *vec) {
  ...
  for (i = 0; i < vec->size / 2; i++) {
    SWAP(vec->buffer[i], vec->buffer[vec->size - i - 1], VectorDataType);
  }
}
...
void vector_swap(struct Vector *a, struct Vector *b) {
  ...
  SWAP(a->buffer, b->buffer, VectorDataType *);
}
